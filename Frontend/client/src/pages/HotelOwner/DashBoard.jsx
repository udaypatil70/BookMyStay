  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">Monitor your room listings, track bookings and analyze revenue.</p>
      </div>
      ...
      <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
        {stats.map((stat) => (
          <div
            key={stat.label}
            className="group relative bg-white rounded-2xl p-6 shadow-sm border border-slate-200/60 hover:shadow-md hover:border-slate-300/60 transition-all duration-300 overflow-hidden"
          >
        ...
        <div className="px-6 py-5 border-b border-slate-100">
          <div className="flex flex-col gap-1 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <p className="text-sm text-slate-500 mt-0.5">Latest activity and payment status for your rooms.</p>
            </div>
          ...
          <table className="w-full min-w-[640px]">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50/50">
                <th className="py-3 px-6 text-left text-xs font-semibold uppercase tracking-[0.1em] text-slate-400">Guest</th>
                <th className="py-3 px-6 text-left text-xs font-semibold uppercase tracking-[0.1em] text-slate-400 hidden sm:table-cell">Room</th>
                <th className="py-3 px-6 text-left text-xs font-semibold uppercase tracking-[0.1em] text-slate-400 hidden md:table-cell">Date</th>
                <th className="py-3 px-6 text-right text-xs font-semibold uppercase tracking-[0.1em] text-slate-400">Amount</th>
                  ...
                  <tr key={index} className="hover:bg-slate-50/50 transition-colors duration-150">
                    <td className="py-4 px-6">
                      <div className="flex items-center gap-3">
                        <div className="w-9 h-9 rounded-full bg-gradient-to-br from-indigo-500 to-indigo-600 flex items-center justify-center text-white text-sm font-semibold shadow-sm">
                          {item.user?.username?.charAt(0)?.toUpperCase() || "G"}
                        </div>
                        <div>
                          <p className="text-sm font-semibold text-slate-800">{item.user?.username || "Guest"}</p>
